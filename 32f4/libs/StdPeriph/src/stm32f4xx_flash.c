...
void FLASH_Unlock(void)
{
	...
	if((FLASH->CR & FLASH_CR_LOCK) != RESET)
	{
		...
		printf("ready unlock flash\n\r");
		FLASH->KEYR = FLASH_KEY1;
		FLASH->KEYR = FLASH_KEY2;
		printf("unlock flash OK\n\r");
	}
	if ((FLASH->CR & 0xFF) & 0x80)
		printf("Programming sequence error\n\r");
	if ((FLASH->CR & 0xFF) & 0x40)
		printf("Programming parallelism error\n\r");
	if ((FLASH->CR & 0xFF) & 0x20)
		printf("Programming alignment error\n\r");
	if ((FLASH->CR & 0xFF) & 0x10)
		printf("Write protection error\n\r");
	if ((FLASH->CR & 0xFF) & 0x02)
		printf("Operation error\n\r");
	if ((FLASH->CR & 0xFF) & 0x01)
